...
import 'package:mobile_medici/reorderables/src/widgets/reorderable_widget.dart';
...
//      '_childIndexToDisplayIndex:$_childIndexToDisplayIndex _childDisplayIndexToIndex:$_childDisplayIndexToIndex _childRunIndexes:$_childRunIndexes _nextChildRunIndexes:$_nextChildRunIndexes');
    ...
    for (int i = 0; i < widget.children.length; i++) {
      var children = widget.children[i];
      wrappedChildren.add(_wrap(children, i));
    }
    if (_dragStartIndex >= 0 &&
        _currentDisplayIndex >= 0 &&
        _dragStartIndex != _currentDisplayIndex) {
      //we are dragging an widget
      var children = widget.children[_currentDisplayIndex];
    ...
    /*for (var i = 0; i < wrappedChildren.length; i++) {
      final children = wrappedChildren[i];
      if (children is WrapCardWidget) {
        if (children.position != -1 && children.position != i) {
          wrappedChildren.insert(
              children.position, wrappedChildren.removeAt(i));
          widget.children.insert(
              children.position, widget.children.removeAt(i));
        }
      }
    }*/
